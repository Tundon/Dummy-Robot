...
uint8_t isEnable(uint8_t id, uint8_t isBlock) {
  ...
  /* 阻塞式通信 */
  while ((pSCA->paraCache.R_Power_State != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
/**
  ...
  *			其他通信错误参见 SCA_Error 错误列表
  */
uint8_t activateActuatorMode(uint8_t id,
...
uint8_t getErrorCode(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Error_Code != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getPosition(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行成功 */
  while ((pSCA->paraCache.R_Position_Real != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanPowertime) return SCA_OperationFailed;
...
uint8_t getPositionKp(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Position_Filter_P != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getPositionKi(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Position_Filter_I != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getPositionUmax(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Position_Filter_Limit_H != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getPositionUmin(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Position_Filter_Limit_L != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getPositionOffset(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Position_Offset != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getMaximumPosition(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Position_Limit_H != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getMinimumPosition(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Position_Limit_L != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t isPositionLimitEnable(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Position_Limit_State != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t isPositionFilterEnable(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Position_Filter_State != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t setPositionCutoffFrequency(uint8_t id,
  ...
  /* 等待执行结果 */
  while ((pSCA->Position_Filter_Value != frequency)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getPositionCutoffFrequency(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Position_Filter_Value != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t clearHomingInfo(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.W_ClearHome != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t setProfilePositionAcceleration(uint8_t id,
  ...
  /* 等待执行结果 */
  while ((pSCA->PP_Max_Acceleration != acceleration)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getProfilePositionAcceleration(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_PP_Max_Acceleration != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t setProfilePositionDeceleration(uint8_t id,
  ...
  /* 等待执行结果 */
  while ((pSCA->PP_Max_Deceleration != deceleration)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getProfilePositionDeceleration(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_PP_Max_Deceleration != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getProfilePositionMaxVelocity(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_PP_Max_Velocity != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getVelocity(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Velocity_Real != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getVelocityKp(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Velocity_Filter_P != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getVelocityKi(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Velocity_Filter_I != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getVelocityUmax(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Velocity_Filter_Limit_H != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getVelocityUmin(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Velocity_Filter_Limit_L != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t isVelocityFilterEnable(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Velocity_Filter_State != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getVelocityCutoffFrequency(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Velocity_Filter_Value != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t setVelocityCutoffFrequency(uint8_t id,
  ...
  /* 等待执行结果 */
  while ((pSCA->Velocity_Filter_Value != frequency)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getVelocityLimit(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Velocity_Limit != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t setProfileVelocityAcceleration(uint8_t id,
  ...
  /* 等待执行结果 */
  while ((pSCA->PV_Max_Acceleration != acceleration)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getProfileVelocityAcceleration(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_PV_Max_Acceleration != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t setProfileVelocityDeceleration(uint8_t id,
  ...
  /* 等待执行结果 */
  while ((pSCA->PV_Max_Deceleration != deceleration)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getProfileVelocityDeceleration(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_PV_Max_Deceleration != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getProfileVelocityMaxVelocity(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_PV_Max_Velocity != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getCurrent(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Current_Real != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getCurrentKp(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Current_Filter_P != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getCurrentKi(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Current_Filter_I != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getCurrentRange(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Current_Max != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t isCurrentFilterEnable(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Current_Filter_State != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getCurrentCutoffFrequency(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Current_Filter_Value != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
/**
  ...
  *			其他通信错误参见 SCA_Error 错误列表
  */
uint8_t setCurrentCutoffFrequency(uint8_t id,
                                  float frequency,
                                  uint8_t isBlock) {
  uint8_t Error;
  uint32_t waitime = 0;
  ...
  /* 等待执行结果 */
  while ((pSCA->Current_Filter_Value != frequency)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getCurrentLimit(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Current_Limit != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getVoltage(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Voltage != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getLockEnergy(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Blocked_Energy != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getMotorTemperature(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Motor_Temp != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getInverterTemperature(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Inverter_Temp != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getMotorProtectedTemperature(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Inverter_Protect_Temp != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getMotorRecoveryTemperature(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Motor_Recover_Temp != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getInverterRecoveryTemperature(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Inverter_Recover_Temp != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
/**
  ...
  *			其他通信错误参见 SCA_Error 错误列表
  */
uint8_t setInverterRecoveryTemperature(uint8_t id,
                                       float temp,
                                       uint8_t isBlock) {
  uint8_t Error;
  uint32_t waitime = 0;
...
uint8_t getActuatorSerialNumber(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Serial_Num != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
...
uint8_t getActuatorLastState(uint8_t id, uint8_t isBlock) {
  ...
  /* 等待执行结果 */
  while ((pSCA->paraCache.R_Last_State != Actr_Enable)
      && (waitime++ < CanOvertime));
  if (waitime >= CanOvertime) return SCA_OperationFailed;
